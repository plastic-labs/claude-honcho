 ...
 */
export function renderHonchoSmile() {
    const p = c.peach;
    const r = c.reset;
    return [
        `  ${p}${B.lower}${B.full}${B.full}${B.full}${B.full}${B.lower}${r}  `,
 ...
 * Wider honcho
 */
export function renderHonchoWide() {
    const p = c.peach;
    const r = c.reset;
    return [
 ...
 * Compact honcho
 */
export function renderHonchoCompact() {
    const p = c.peach;
    const r = c.reset;
    return [
 ...
 */
export function renderHonchoGradient() {
 ...
 */
export function renderHonchoMinimal() {
    const p = c.peach;
    const r = c.reset;
    return [
 ...
 */
export function renderHonchoAscii() {
    const o = c.orange;
    const p = c.peach;
    const s = c.salmon;
    const e = c.dark;
 ...
 */
export function renderHonchoAsciiStandard() {
    const o = c.orange;
    const p = c.peach;
    const s = c.salmon;
    const e = c.dark;
 ...
 */
export function renderHonchoAsciiCompact() {
    const o = c.orange;
    const p = c.peach;
    const s = c.salmon;
    const e = c.dark;
 ...
 */
function writeTTY(text, switchToUtf8 = false) {
    try {
 ...
 */
export function displayHonchoStartup(label, subtitle, extra) {
    ...
    const output = lines.map((line, i) => {
        ...
        if (i === 0)
            return `${padded}  ${c.pale}${labelText}${c.reset}`;
        if (i === 1)
            return `${padded}  ${c.dim}${subtitleText}${c.reset}`;
        if (i === 2 && extra)
            return `${padded}  ${c.dim}${extra}${c.reset}`;
        return padded;
    });
    return output.join("\n");
}
 ...
 */
export function displayHonchoStartupTTY(label, subtitle, extra) {
 ...
 */
export function getHonchoLines() {
