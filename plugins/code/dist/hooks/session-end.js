...
import { getQueuedMessages, markMessagesUploaded, generateClaudeSummary, saveClaudeLocalContext, loadClaudeLocalContext, getClaudeInstanceId, } from "../cache.js";
    ...
    try {
        ...
        if (queuedMessages.length > 0) {
            ...
            if (validMessages.length > 0) {
                ...
                const userMessages = validMessages.map((msg) => userPeer.message(msg.content, {
                    metadata: {
        ...
        if (config.saveMessages !== false && transcriptMessages.length > 0) {
            ...
            if (assistantMessages.length > 0) {
                ...
                const messagesToSend = assistantMessages.map((msg) => claudePeer.message(msg.content, {
                    metadata: {
        ...
        await session.addMessages([
            claudePeer.message(`[Session ended] Reason: ${reason}, Messages: ${transcriptMessages.length}, Time: ${new Date().toISOString()}`, {
                metadata: {
                    instance_id: instanceId || undefined,
                    session_affinity: sessionName,
                },
