...
import { getCachedUserContext, isContextCacheStale, setCachedUserContext, queueMessage, incrementMessageCount, shouldRefreshKnowledgeGraph, markKnowledgeGraphRefreshed, getClaudeInstanceId, } from "../cache.js";
...
export async function handleUserPrompt() {
    ...
    if (shouldSkipContextRetrieval(prompt)) {
        ...
        if (uploadPromise)
            await uploadPromise.catch((e) => logHook("user-prompt", `Upload failed: ${e}`, { error: String(e) }));
        process.exit(0);
    }
...
async function uploadMessageAsync(config, cwd, prompt) {
...
async function fetchFreshContext(config, cwd, prompt) {
    const honcho = new Honcho(getHonchoClientOptions(config));
    const sessionName = getSessionName(cwd);
    ...
    if (contextResult) {
        ...
        const rep = contextResult.representation;
        ...
        const peerCard = contextResult.peerCard || contextResult.peer_card;
        if (peerCard?.length) {
...
function outputContext(peerName, contextParts) {
    const output = {
        hookSpecificOutput: {
