...
import { setCachedUserContext, setCachedClaudeContext, loadClaudeLocalContext, resetMessageCount, setClaudeInstanceId, getCachedGitState, setCachedGitState, detectGitChanges, } from "../cache.js";
...
function formatRepresentation(rep) {
    const parts = [];
    ...
    if (rep?.deductive?.length > 0) {
        ...
        const deductive = sorted
            ...
            .map((d) => `- ${d.conclusion} (from: ${d.premises?.join(", ") || "prior observations"})`)
            .join("\n");
    ...
    try {
        ...
        if (gitChanges.length > 0) {
            const gitObservations = gitChanges
                ...
                .map((change) => userPeer.message(`[Git External] ${change.description}`, {
                metadata: {
                    type: "git_change",
...
- Directory: ${cwd}`;
        ...
        const [userContextResult, claudeContextResult, summariesResult, userChatResult, claudeChatResult] = await Promise.allSettled([
            ...
            userPeer.chat(`Summarize what you know about ${config.peerName} in 2-3 sentences. Focus on their preferences, current projects, and working style.${branchContext}${changeContext}${featureHint}`, { session }),
            // 5. Dialectic: Ask about claude (self-reflection, context-enhanced)
            claudePeer.chat(`What has ${config.claudePeer} been working on recently?${branchContext}${featureHint} Summarize the AI assistant's recent activities and focus areas relevant to the current work context.`, { session }),
        ]);
        // Log async results
        ...
        const userChatContent = userChatResult.status === "fulfilled"
            ? (typeof userChatResult.value === "string" ? userChatResult.value : userChatResult.value?.content)
            : null;
        if (userChatContent) {
            contextParts.push(`## AI Summary of ${config.peerName}\n${userChatContent}`);
        }
        const claudeChatContent = claudeChatResult.status === "fulfilled"
            ? (typeof claudeChatResult.value === "string" ? claudeChatResult.value : claudeChatResult.value?.content)
            : null;
