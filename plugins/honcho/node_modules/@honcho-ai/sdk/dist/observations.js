...
exports.ObservationScope = exports.Observation = void 0;
...
class Observation {
    ...
    static fromApiResponse(data) {
        return new Observation(data.id ?? '', data.content ?? '', data.observer_id ?? '', data.observed_id ?? '', data.session_id ?? '', data.created_at ?? '');
    }
    /**
...
class ObservationScope {
     ...
     */
    constructor(client, workspaceId, observer, observed) {
        this._client = client;
     ...
     */
    async list(page = 1, size = 50, session) {
        ...
        const response = await this._client.workspaces.observations.list(this.workspaceId, {
            filters,
            page,
            size,
        });
        return (response.items ?? []).map((item) => Observation.fromApiResponse(item));
    }
    /**
    ...
    async query(query, topK = 10, distance) {
        ...
        // biome-ignore lint/suspicious/noExplicitAny: SDK workspaces type doesn't include observations
        const response = await this._client.workspaces.observations.query(this.workspaceId, {
            query,
            top_k: topK,
            distance,
            filters,
     ...
     */
    async delete(observationId) {
        // biome-ignore lint/suspicious/noExplicitAny: SDK workspaces type doesn't include observations
        await this._client.workspaces.observations.delete(this.workspaceId, observationId);
    }
    /**
    ...
    async create(observations) {
        ...
        const requestObservations = observationArray.map((obs) => ({
            content: obs.content,
            session_id: typeof obs.sessionId === 'string' ? obs.sessionId : obs.sessionId.id,
            observer_id: this.observer,
            observed_id: this.observed,
        }));
        // biome-ignore lint/suspicious/noExplicitAny: SDK workspaces type doesn't include observations
        const response = await this._client.workspaces.observations.create(this.workspaceId, { observations: requestObservations });
        return (response ?? []).map((item) => Observation.fromApiResponse(item));
    }
    /**
     ...
     */
    async getRepresentation(options) {
        const response = await this._client.workspaces.peers.workingRepresentation(this.workspaceId, this.observer, {
        ...
        return representation_1.Representation.fromData(rep);
    }
    /**
...
exports.ObservationScope = ObservationScope;
