...
import { findTranscripts } from "./transcript-parse.js";
...
// entryUrl is the runner entry's location, from which ../../scripts holds the
// renderer in both the source tree and the staged dist/.
function installStatusline(entryUrl: string): void {
  console.log(s.section("Installing memory statusLine"));

  const src = join(dirname(fileURLToPath(entryUrl)), "..", "..", "scripts", "honcho-statusline.sh");
...
export async function main(entryUrl: string): Promise<void> {
